...
export default function TestGameContainer() {
  // Use wagmi hooks
  ...
  useEffect(() => {
    ...
    } else if (metaMaskAccount && metaMaskBalance) {
      setGameState(prev => ({
        ...prev,
        balance: parseFloat(metaMaskBalance)
      }));
    }
  }, [walletBalance, metaMaskAccount, metaMaskBalance, setGameState]);
  ...
  const fetchLastGameResult = async () => {
    ...
    // Use wagmi address, MetaMask account, or try to get directly
    let playerAddress = address as `0x${string}` | undefined || metaMaskAccount as `0x${string}` | undefined;
  ...
  const startGame = async () => {
    ...
    // Even if wagmi shows as not connected, we'll try to get the address directly
    ...
    try {
      ...
      try {
        // Create token contract instance with ethers v5.7.2
        const provider = new ethers.providers.Web3Provider(window.ethereum as ExtendedProvider);
        const signer = provider.getSigner();
      ...
      let txHash;
      try {
        // Create game contract instance with ethers v5.7.2
        const gameProvider = new ethers.providers.Web3Provider(window.ethereum as ExtendedProvider);
        ...
        if (result) {
          // Play the game animation based on the blockchain result
    ...
    <React.Fragment>
      <div className="w-full flex flex-col items-center">
        ...
        {!isConnected && !metaMaskAccount && (
          <div className="p-4 bg-gray-800 border-b border-gray-700 flex justify-center">
